...
// JournalHandler は仕訳管理のエンドポイント。
type JournalHandler struct {
	journals usecase.JournalUsecase
}

// NewJournalHandler は JournalHandler を生成する。
...
func (h *JournalHandler) AddBatch(w http.ResponseWriter, r *http.Request) {
	...
	writeJSON(w, http.StatusCreated, map[string]any{"IDs": ids, "Warnings": warnings})
}
...
func (h *JournalHandler) Search(w http.ResponseWriter, r *http.Request) {
	...
	writeJSON(w, http.StatusOK, map[string]any{"Journals": entries, "TotalCount": total})
}

// parseSearchQuery はクエリパラメータから検索条件を組み立てる。
...
func (h *JournalHandler) CheckDuplicates(w http.ResponseWriter, r *http.Request) {
	...
	if v := r.URL.Query().Get("threshold"); v != "" {
		if threshold, err = strconv.Atoi(v); err != nil {
			writeError(w, badQuery("threshold", v))
			return
		}
	}
